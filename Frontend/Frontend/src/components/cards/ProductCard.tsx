...
export function ProductCard({
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            {isExpired
              ...
              : isExpiringSoon
              ? `⚠️ Vence pronto (${dayjs(expiration_date).format(
                  "DD/MM/YYYY"
                )})`
              : ""}
          </span>
      ...
      {isLowerStock && isActive && (
        <div
          className={`px-4 py-2 flex items-center space-x-2 ${
            isLowerStock ? "bg-red-100" : "bg-orange-100"
          }`}
        >
          <FiAlertTriangle
            className={`h-4 w-4 ${
              isLowerStock ? "text-yellow-600" : "text-orange-600"
            }`}
          />
          <span
            className={`text-sm font-medium ${
              isLowerStock ? "text-yellow-800" : "text-orange-800"
            }`}
          >
            {isLowerStock
      ...
      <div className="p-5">
        ...
        <div className="mb-4 text-sm text-gray-700 space-y-1">{features}</div>
